require([ 'libraries/jquery' ], function(jQuery) {
    (function(jQuery) {
        ...
        function indexify(i, length) {
            return (i >= length) ? indexify(i - length, length)
                    : ((i < 0) ? indexify(i + length, length) : i);
        }
        ...
        function checkFamily(content, index) {
            ...
            if (content[index[0]]) {
                if (content[index[0]].text) {
                    ...
                    return checkFamily(content[newIndex[0]], newIndex
                            .splice(1, newIndex.length));
                }
            } else {
                return false;
            }
        ...
        function buildIndex(content, index) {
            ...
            if (obj === false) {
                return buildIndex(content, incrementIndex(index.slice(
                        0, index.length - 1)));
            } else if (obj === true) {
        ...
        function buildFamily(elem, content, index, data) {
            ...
            return buildFamily(newElem, content[index[0]], newIndex,
                    data);
        }
        ...
        function ticker(elem, threadIndex, data) {
            ...
            if (data.cursorIndex !== false) {
                data.cursorIndex = indexify(data.cursorIndex + 1,
                        data.cursorList.length);
        ...
        function tape(elem, threadIndex) {
            var data = elem.data(name);
            if (data.eventIndex == threadIndex) {
                data.elemIndex = buildIndex(data.content,
                        data.elemIndex);
                if (data.elemIndex === false) {
                    return setTimeout(function() {
                        if (data.running
                                && (data.eventIndex == threadIndex)) {
                            advanceItem(elem);
                            return tape(elem, threadIndex);
                        }
                        threadIndex = null;
                ...
                jQuery.extend(data, buildFamily(elem, data.content,
                        data.elemIndex));
        ...
        jQuery.fn[name] = function(options) {
            var o = jQuery.extend({}, jQuery.fn.ticker.defaults,
                    options);
            return this
            .each(function() {
                ...
                .data(name, {
                    ...
                    cursorIndex : (typeof (o.cursorList) == "object") ? 0
                            : false,
                            nextItem : 0,
                            eventIndex : 0
                })
